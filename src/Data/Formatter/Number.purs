...
format (Formatter f) num =
  let
    absed = Math.abs num
    tens =
      if absed > 0.0 then max (Int.floor $ Math.log absed / Math.ln10) 0
      else 0
  in
...
unformatParser (Formatter f) = do
  ...
  before <-
    if Arr.length beforeDigits < f.before then P.fail "Error: too few digits before dot"
    else pure $ Int.toNumber $ foldDigits beforeDigits

  afterDigits <- some parseDigit
  after <-
    if Arr.length afterDigits < f.after then P.fail "Error: too few digits after dot"
